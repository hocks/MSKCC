...
Patch0: pbs_server_init_d.patch
Patch1: fix_mom_priv_3.0.5.patch
...
# fix the tail bug in the init script
/bin/sed -i 's/tail *+2/tail -n +2/g' src/tools/init.d/pbs
...
./configure --prefix=/opt/%{name}  \
	...
	--with-rcp=scp --enable-syslog --enable-debug \
	--disable-gcc-warnings --enable-docs \
	--with-pam=%{pam_dir} \
	--enable-cpuset --enable-nvidia-gpus 

##	--with-nvml-include=/usr/local/cuda/nvml/include \
##	--with-nvml-lib=/usr/local/cuda/nvml/lib64 
